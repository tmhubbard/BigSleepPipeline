...
# This script contains... a lot, really. It's my (Trevor Hubbard) modification
# of BigSleep, which also contains Big-GAN and CLIP, so... yeah, lots of code.
# This is just me trying to pick it apart a little bit - restructure it, add my
# own code, and experiment with it in some ways. 
...
import glob
from content.CLIP.clip import clip
...
# HELPER FUNCTIONS
# ============================
# These functions are just a couple of helper functions! 

def displ(img, imName, output, pre_scaled=True):
  img = np.array(img)[:,:,:]
  img = np.transpose(img, (1, 2, 0))
  if not pre_scaled:
    img = scale(img, 48*4, 32*4)
  imageio.imwrite(output + "/" + imName + '.png', np.array(img))
...
# GENERATOR FUNCTIONS
# ============================
# These are functions that were used for the Generator! 
...
# TRAINING FUNCTIONS
# ===========================
# These are functions that helped out with the training process

def checkin(loss, curIter):
  # print(loss)
  best = torch.topk(loss[2], k=1, largest=False)[1]
  with torch.no_grad():
    al = model(*lats(), 1)[best:best+1].cpu().numpy()
  for allls in al:
    displ(allls, str(curIter), outputPathName)
...
# GENERATOR CLASSES
# ==========================
# These are classes that are used for the Generator
...
# LATENT COORDINATES
# ==========================
# These are methods used with the "latent coordinates"
...
class Pars(torch.nn.Module):
    def __init__(self):
        ...
        # I *believe* I could change the initialization of this to start at a particular latent coordinate
...
# Grab the parameters from the command line arguements
inputSeed = int(sys.argv[1])
inputLearnRate = float(sys.argv[2])
amtImg = int(sys.argv[3])
userInput = " ".join(sys.argv[4].split("_"))
skipAmt = int(sys.argv[5])
print("\n\nInput seed is %d" % inputSeed)
print("Input learnRate is %.3f" % inputLearnRate)
print("User input is %s" % userInput)

# Setting the manual seed
firstPath = "Output\\" + userInput
if (not Path.exists(Path(firstPath))):
  Path.mkdir(Path(firstPath))

foundNewPath = False
versionCt = 0
outputPathName = ""

outputPathName = firstPath + "/seed " + str(inputSeed)
if (not Path.exists(Path(outputPathName))):
  Path.mkdir(Path(outputPathName))

# This statement supports creating multiple versions of the same seed 
else:
  # Search for the highest occurring number
  maxNum = 1
  for folder in Path(firstPath).iterdir():
    if (folder.is_dir()):
      folderName = str(outputPathName)
      if (str(Path(outputPathName)) in str(folder)):
        splitName = str(folder).split("-")
        if (len(splitName) > 1):
          print(splitName[-1])
          if(splitName[-1].strip()[0]=="v"):
            curVersion = int(splitName[-1].strip()[1:])
            if (curVersion >= maxNum):
              maxNum = curVersion + 1

  outputPathName += " - v" + str(maxNum)
  Path.mkdir(Path(outputPathName))
...
# get_ipython().run_line_magic('cd', 'CLIP')
...
torch.manual_seed(inputSeed)

# ==============================
# LATENET COORDINATE MAIN
# ==============================

lats = Pars().cuda()
optimizer = torch.optim.Adam(lats.parameters(), inputLearnRate)
eps = 0
...
with torch.no_grad():
  ...
  #print(lats())
  for allls in al:
    displ(allls, "starting", outputPathName)
    #print('\n')
...
itt = 0
iterAmt = amtImg
for i in range(iterAmt):
    if (((i % (iterAmt/10)) == 0)):
        print("%.2f%%" % ((i/iterAmt)*100))
    train(eps, i)
    itt+=1
# with open("last lat.txt", "w") as latFile:
#   torch.set_printoptions(threshold=5000)
#   print(lats()[0].shape)
#   latFile.write(str(lats()[0]))
